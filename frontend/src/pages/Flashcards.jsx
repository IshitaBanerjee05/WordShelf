...
export default function Flashcards() {
  ...
  const handleNext = async (correct) => {
    ...
    if (activeCard) {
      try {
        await api.post(`/vocabulary/${activeCard.id}/review`, { quality: correct ? 4 : 2 });
      } catch (err) {
  ...
  // Re-fetch from backend instead of replaying the same in-memory list
        ...
        <div className="flex-1 relative flex items-center justify-center mb-8">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="w-full max-w-2xl h-96 bg-white rounded-3xl shadow-xl border border-slate-100 p-12 flex flex-col justify-center items-center text-center cursor-pointer relative overflow-hidden hover:shadow-2xl transition-shadow"
              onClick={() => setIsFlipped(!isFlipped)}
            >
              <div className="absolute -top-24 -right-24 w-48 h-48 bg-primary-50 rounded-full opacity-50" />
              <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-emerald-50 rounded-full opacity-50" />

              {!isFlipped ? (
                <div>
                  <span className="text-sm font-bold text-primary-500 tracking-widest uppercase mb-6 block">Word</span>
                  <h3 className="text-6xl font-extrabold text-slate-900 font-serif tracking-tight">{activeCard.front}</h3>
                  <p className="mt-8 text-slate-400 text-sm font-medium flex items-center gap-2 justify-center">
                    <svg className="w-5 h-5 animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 15l-3 3m0 0l-3-3m3 3V8" />
                    </svg>
                ...
                <div className="w-full">
                  ...
                  {activeCard.example && (
                    <>
                      <div className="w-16 h-1 bg-slate-200 mx-auto my-6 rounded-full" />
