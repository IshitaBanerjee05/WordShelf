...
import { BookMarked, Home, Library, BookOpenCheck, User, LogOut } from "lucide-react";
...
export default function Layout() {
  ...
  return (
    <div className="flex bg-gray-50 min-h-screen">
      <aside className="fixed inset-y-0 z-50 flex w-72 flex-col bg-slate-900 text-white shadow-xl">
            ...
            return (
              <Link key={item.name} to={item.href}
                className={cn(
                  ...
                  isActive ? "bg-primary-600 shadow-md shadow-primary-900/50 text-white"
                           : "text-slate-300 hover:bg-slate-800 hover:text-white"
                )}>
                <Icon className={cn("mr-3 h-5 w-5 flex-shrink-0 transition-transform duration-200",
        ...
        <div className="p-4 border-t border-slate-800">
          {/* Profile link (replaces old Settings) */}
          <Link to="/profile"
            className={cn(
            ...
            <User className="mr-3 h-5 w-5 text-slate-400 group-hover:text-white" />
            Profile & Settings
          </Link>
        ...
        <header className="h-16 flex items-center justify-between border-b border-gray-200 bg-white/80 backdrop-blur-md px-8 sticky top-0 z-40">
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-gray-900 capitalize tracking-tight">
              {location.pathname === '/profile' ? 'Profile & Settings' : location.pathname.split('/').pop() || 'Dashboard'}
            </h1>
          </div>
